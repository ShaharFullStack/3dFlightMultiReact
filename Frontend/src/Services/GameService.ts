...
export class GameService {
    ...
    private createInitialPlane(): void {
        ...
        try {
            ...
            if (this.currentPlane) {
                ...
                (window as any).currentPlane = this.currentPlane;
            }
        } catch (error) {
            console.error("Error creating initial plane:", error);
            // Fallback plane already exists
        }
    }

    /**
     * Create environment (river, houses, clouds, balloons, etc.)
     */
    ...
    public createEnhancedExplosion(position: THREE.Vector3): void {
        ...
        try {
            ...
            const expandExplosion = () => {
                ...
                if ((explosion.material as THREE.MeshBasicMaterial).opacity > 0) {
    ...
    /**
     * Mouse controls setup (pointer lock, etc.)
     */
    ...
    /**
     * Create a balloon (either from server data or locally)
     */
    ...
    /**
     * Send bullet creation to server (if multiplayer)
     */
    ...
    private updateRemotePlayerHealthBar(playerId: string): void {
        ...
        try {
            ...
            if (context) {
                ...
                context.fillRect(2, 2, Math.max(0, health * 0.96), 6); 
            }
    ...
    public createRemoteBullet(bullet: BulletModel): void {
        ...
        try {
            ...
            // שמירת מידע ב-userData
            bulletMesh.userData.bulletId = bullet.id;
            ...
            // שמירת רפרנס ל-Mesh באובייקט ה-BulletModel עצמו
            ...
            if (bullet.playerId && this.remotePlayers[bullet.playerId]) {
                ...
                this.remotePlayers[bullet.playerId].bullets.push(bullet);
            }

            // הפעלת בדיקות התנגשות
    ...
    public updateRemoteBullets(playerId: string, bullets: any[]): void {
        ...
        try {
            ...
            if (remotePlayer.bullets && remotePlayer.bullets.length > 0) {
                remotePlayer.bullets.forEach(oldBullet => {
                    if (oldBullet.mesh && this.scene) {
            ...
            bullets.forEach(bulletData => {
                if (bulletData) {
                    const bulletModel: BulletModel = {
                        ...
                        mesh: undefined // יתמלא בתוך createRemoteBullet
                    };
                    this.createRemoteBullet(bulletModel);
                }
            });
        } catch (error) {
    ...
    /**
     * בדיקת פגיעת כדור בשחקן מקומי
     */
    private detectBulletCollisions(bulletMesh: THREE.Mesh): void {
        if (!bulletMesh || !this.scene) return;

        const checkCollisions = () => {
            // אם ה-Mesh כבר הוסר מהסצנה, אין מה לבדוק
            if (!this.scene?.getObjectById(bulletMesh.id)) return;

            try {
                // אם הכדור לא שייך לשחקן המקומי, בודקים התנגשות בשחקן המקומי
                if (bulletMesh.userData.playerId !== networkClient.getPlayerId() && this.player) {
                    const distance = bulletMesh.position.distanceTo(this.player.position);
                    ...
                    if (distance < 15) {
                        ...
                        this.createEnhancedExplosion(bulletMesh.position.clone());
                        return;
                    }
                }

                // ממשיכים לבדוק בשפריים הבא כל עוד ה-Mesh חי בסצנה
                if (this.scene?.getObjectById(bulletMesh.id)) {
